...
Res<-c()
   for(i in 1982:2017) {
		...
		Last_snow_date<-as.numeric(format(c(date[ind][
		               apply(M, 1, FUN=function(x) which(x==0)[1])-1]), '%j'))
########################
# temporary addition	
        Last_snow_date[is.na(Last_snow_date)]<-max(Last_snow_date, na.rm=TRUE)+7
########################		
		Mean_all<-mean(Last_snow_date)
...
# we now scale and center predicted from stayover duration and lugworm abundance mass by SEX
...
lines(ucl~Male_weight_cov, data=subset(Estimates_males, Male_weight_cov>=
     min(Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='mWad']) &
     Male_weight_cov<=
     max(Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='mWad'])), lwd=2, lty=2)
...
lines(ucl~Female_weight_cov, data=subset(Estimates_females, Female_weight_cov>=
     min(Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='fWad']) &
     Female_weight_cov<=
     max(Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='fWad'])) , lwd=2, lty=2)
