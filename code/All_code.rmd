...
write.csv(Res, file='Snow_remote.csv', row.names=FALSE)
...
if (Use_local_data) {
   ...
   Snow_remote<-read.csv('https://git.io/vXAZ1')
...
if (Use_local_data) {
   ...
   # read Snow
   Snow<-read.csv('https://git.io/vXAZ1', as.is=TRUE)
   # read breeding data
...
contour(as.image(x=XY[1:2], Z=XY$Phi_females),add=T, levels=round(Females_average_survival,2),
...
```{r}
   ...
   Capture_data_males<-subset(Capture_data, Sex==1)
   Capture_data_females<-subset(Capture_data, Sex==2)
...
```{r fig.width=2, fig.height=2}
   ...
   annotate("segment", x=X_labels, xend=X_labels, y=0-0.075, yend=0+0.075,
            col='#E69F00', lwd=1.5)+
   annotate("text", x=X_text, y = 0, label = "arrival weight (95%)",hjust = 0) +
