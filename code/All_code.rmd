...
This script can operate online or local data. If `Use_local_data` set to TRUE working directory expected to have folders `data`. If `Run_everything` set to TRUE `results` directory will be created and all results will be saved there.
```{r}
...
if (Use_local_data) if (!'data' %in% list.dirs(full.names=FALSE, recursive=FALSE)) { 
...
Fuelling_slopes<-data.frame(Yr=Years,
         ...
         Fuelling_slope_females_predicted=13.4*Lugworms_vec/(fuelling_model$BUGSoutput$mean$K_sat_f[1]+Lugworms_vec))
write.csv(Fuelling_slopes, file='./results/Fuelling_slopes.csv', row.names=FALSE)
```

### 5.8 Plot estimated refuelling slopes by lugworm abundance
...
Y_fem<- t(sapply(XX, FUN=function(x)
		 ...
		 probs=c(0.025, 0.5,0.975))))
lines(Y_fem[,1]~XX, lwd=2, lty=2)
lines(Y_fem[,2]~XX, lwd=2)
lines(Y_fem[,3]~XX, lwd=2, lty=2)
for (i in 1:length(Lugworms_vec)) {
...
All_staging_data$Weight_gain_females_st<-scale(All_staging_data$Departure_fuel_load_females)
All_staging_data$Weight_gain_males_st<-scale(All_staging_data$Departure_fuel_load_males)

All_staging_data$Weight_gain_pr_females_st<-
                                   scale(All_staging_data$Departure_fuel_load_females_predicted)
All_staging_data$Weight_gain_pr_males_st<-
                                   scale(All_staging_data$Departure_fuel_load_males_predicted)

Departure_Weight_gain_Females<- cbind( group='f', subset(All_staging_data,
    select=c(Yr, Departure_fuel_load_females, Departure_fuel_load_females_predicted,  Weight_gain_females_st,  Weight_gain_pr_females_st)))
names(Departure_Weight_gain_Females)[3]<-'Weight_gain'
names(Departure_Weight_gain_Females)[4]<-'Weight_gain_predicted'
names(Departure_Weight_gain_Females)[5]<-'Weight_gain_st'
names(Departure_Weight_gain_Females)[6]<-'Weight_gain_pr_st'

Departure_Weight_gain_Males<- cbind(group='m', subset(All_staging_data,
    select=c(Yr, Departure_fuel_load_males, Departure_fuel_load_males_predicted, Weight_gain_males_st, Weight_gain_pr_males_st)))
names(Departure_Weight_gain_Males)[3]<-'Weight_gain'
names(Departure_Weight_gain_Males)[4]<-'Weight_gain_predicted'
names(Departure_Weight_gain_Males)[5]<-'Weight_gain_st'
names(Departure_Weight_gain_Males)[6]<-'Weight_gain_pr_st'
...
# we now scale and center mass by SEX
...
Phi.weight_gain.SEX.plus.TSM=list(formula=~two_plus + Weight_gain_pr_st*SEX)
...
### 6.5 Prediction from the best model
```{r}
Model<-Res_adj[[5]]
Male_weight_pr<-Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='m']
...
Female_weight_pr<-Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='f']
...
Design<-matrix(nrow=1, ncol=20, data=0)
Design[,1]<-1
Design[,2]<-1
Design[,4]<-1
...
write.csv(Estimates_females, file='./results/female_survival_prediction.csv', row.names=TRUE)
write.csv(Estimates_males, file='./results/male_survival_prediction.csv', row.names=TRUE)
```

### 6.6 Plot survival over departure fuel load
```{r fig.width=10, fig.height=5}
if (Save_pdf) pdf('male_female_survival_over_fuel_load.pdf')

par(mfrow=c(1,2))
# Males
plot(estimate~Male_weight_cov, data=Estimates_males,  ylim=c(0.8, 0.97),
     ylab='Annual apparent survival', xlab='Deposited fuel, g', las=1,
     xlim=range(Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='m']),
     type='n', main='Males')
lines(estimate~Male_weight_cov, data=subset(Estimates_males, Male_weight_cov>=
     min(Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='m']) &
     Male_weight_cov<=
     max(Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='m'])), lwd=2)
lines(ucl~Male_weight_cov, data=subset(Estimates_males, Male_weight_cov>=
     min(Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='m']) &
     Male_weight_cov<=
     max(Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='m'])), lwd=2, lty=2)
...
#  Females
plot(estimate~Female_weight_cov, data=Estimates_females,  ylim=c(0.8, 0.97),
     ylab='Annual apppparent survival', xlab='Deposited fuel, g', las=1,
     xlim=range(Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='f']),
     type='n', main='Females')
lines(estimate~Female_weight_cov, data=subset(Estimates_females, Female_weight_cov>=
     min(Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='f']) &
     Female_weight_cov<=
     max(Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='f'])), lwd=2)
lines(ucl~Female_weight_cov, data=subset(Estimates_females, Female_weight_cov>=
     min(Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='f']) &
     Female_weight_cov<=
     max(Departure_Weight_gain$Weight_gain_predicted[Departure_Weight_gain$group=='f'])) , lwd=2, lty=2)
...
```{r}
XY<-expand.grid(Lugworm_density=seq(5,30, 0.05), stayover=seq(17,37, 0.05))
```
now for every of these points I get estimate of fuelling slope, that I will combine with the stayover
```{r}
x=XY[,1]
...
```
Estimate mass gain from slopes and stayovers
```{r}
...
plot(Stayovers$Stayover~Stayovers$Yr, type='n', ylim=c(17, 36),  ylab='',
...
axis(4, labels=FALSE)
mtext(side=2, 'refuelling time (days)', line=1,cex=1.5,)
...
image.plot(as.image(x=XY[1:2], Z=XY$Phi_females, nx=200, ny=200), 
          col=Colors(256), xlab='', ylab='', las=1, ylim=c(17,36), xlim=c(5,30),
	  legend.mar=7, cex.axis=1.5, legend.cex=1.5)
Females_slope_average<-approx(x=XY$Lugworm_density, y=XY$Female_Median_Slope, mean(Stayovers$lugworms_ad, na.rm=TRUE) )$y
...
contour(as.image(x=XY[1:2], Z=XY$Phi_females),add=T, levels=c(0.8, 0.85, 0.9, 0.95), col=grey(0.5), labcex=1)

abline(h=predict(lm(Stayover~Yr, data=Stayovers), newdata=data.frame(Yr=c(2017))), lwd=2, col=Late_color)
abline(v=22.9, col=Late_color, lwd=2)
abline(h=predict(lm(Stayover~Yr, data=Stayovers), newdata=data.frame(Yr=c(1995))), lwd=2, col=Early_color)
abline(v=mean(Stayovers$lugworms_ad, na.rm=TRUE), col=Early_color, lwd=2)
...
hist(Stayovers$lugworms_ad, main='', xlab='', ylab='n years', las=1, col='#6FC067',
     axes=FALSE, xaxs='i', xlim=c(5, 30))
axis(2, cex.axis=1.5, las=1)
axis(3, labels=FALSE)
abline(v=mean(Stayovers$lugworms_ad, na.rm=TRUE), lwd=2, col=Early_color)
abline(v=22.9,  lwd=2, col=Late_color)
box()
mtext(side=2, 'n Years', line=3, cex=1.5)
mtext(side=1, expression(paste( plain('Lugworm density (ind./m') ^ 
                                plain("2") ,  plain(')'))), 
								line=3, cex=1.5)
...
## 10. Plot capture data with slopes (Extended data Figure 1 and 2)
...
```{r}
   ...
   Departure_Wadden$Departure<-Departure_Wadden$yday-4
