...
if (Use_local_data) {
   if (!'results' %in% list.dirs(full.names=FALSE, recursive=FALSE)) stop('results direcotry not found!')
   if (!'data' %in% list.dirs(full.names=FALSE, recursive=FALSE)) stop('data direcotry not found!')
}
```
...
I will first plot on screen and second into .pdf
```{r}
par(mfrow=c(1,2))
# Males
...
# and now pdf
pdf('male_female_survival_over_fuel_load.pdf')
...
if (Run_everything) {
   ...
   saveRDS(Counts_model, file='Mauritania_counts_model.RDS')
} else {
   Counts_model<-readRDS(gzcon(url('https://git.io/vXuqH')))
}
```
### 5.5 Plot model results for Panel C at Fig.1
I will plot on the device first and to then to pdf.
```{r}
print(Counts_model)
par(mfrow=c(1,1))
...
```
### 6.3 Plot the figure
Here I will first plot on device and then into .pdf
```{r}
par(mar=c(4,4,2,4))
# snow melt
...
#---------------------------------------------------------------------------------------
# And now the same into pdf
pdf('Taimyr_and_wadden.pdf', width=5, height=10, useDingbats=FALSE)
par(mar=c(4,4,2,4))
# snow melt
plot(yday~Yr, data=Snow, type='n', ylim=c(115, 190), las=1, xlim=c(1993, 2015),axes=FALSE, ylab='')
axis(1)
...
b<-sapply(x, FUN=function(x) quantile(x*11/(fuelling_model$BUGSoutput$sims.list$K_sat_m + x),
                             probs =c(0.025, 0.5, 0.975)))
XY$Male_Median_Slope<-b[2,]
XY$Male_lcl_Slope<-b[1,]
XY$Male_ucl_Slope<-b[3,]
```
Now estimate weight gain from slopes and stayovers
```{r}
XY$Slope_Weight_gain_Females<-XY$Female_Median_Slope * XY$stayover
XY$Slope_Weight_gain_Males<-XY$Male_Median_Slope * XY$stayover
```
### 7.3 Plot 
```{r}
pdf(file='Figure 2_4 graphs_07_Nov_2016.pdf', useDingbats=FALSE, width=10, height=10)
par(mfrow=c(2,2))
...
Early_color<-'#6FC067'
#-------------------------------------------------------------
# plot with years and stayover duration
plot(Stayovers$Stayover~Stayovers$Yr, type='n', ylim=c(17, 37),  ylab='', las=1, xlim=c(1993, 2016), xlab='', cex=1.5, axes=F, yaxs='i')
axis(1, cex.axis=1.5)
...
points(Stayover~Yr, data=Stayovers[i,], pch=21, cex=4, bg=My_colors[round(Stayovers[i,]$time_scales*100)], lwd=2)
