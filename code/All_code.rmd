...
Selected_Points_near_field_station<-Selected_Points[sort(order(spDists(coordinates(Grid_centers_lon_lat[Selected_Points,]), Field_site, longlat=TRUE))[1:2])]
...
plot(jdate50_south~ jdate50_ground, data=Snow_all, pch=19, cex=2, col=Colors[3], las=1, xlab='Snow 50% date from ground observations (yday)', ylab='Snow 50% date from remote data (yday)')
...
While fuelling in the Wadden Sea taymyrensis godwits concentrate near Terschelling Island (53.4°N, 5.34°E),  that is 50 km away from long term lugworm sampling area at Balgazand (52.9°N, 4.87°E). In order to validate use of Balgzand data on lugworm abundance for godwits we used shorter, but covering both areas dataset - Synoptic Intertidal Benthic Survey ([SIBES](https://www.nioz.nl/en/education/internships/assessing-the-macrozoobenthos-of-the-dutch-wadden-sea)) - large scale grid sampling of benthic fauna in the Wadden Sea.  We used SIBES data from sampling stations within 500 meters from Terschelling coast (fuelling area) and from Balgzand for all available years (2008 - 2014). 
```{r}
if (Use_local_data) {
...
```
Data on both sites show correlated changes over years (Pearson's _r_=0.86) allowing us to use Balgzand data as a proxy for fuelling conditions for lugworms.
...
### 3.8 Plot estimated fuelling slopes by lugworm abundance
...
mtext(side=1, expression(paste( plain('Lugworm density (') , frac(plain(' individuals '), plain('meter')^ plain("2")), plain(')'))), line=3, cex=1.5)
