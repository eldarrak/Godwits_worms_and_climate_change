...
if (Use_local_data) {
   ...
   Arr_Wadden<-read.csv('https://git.io/vXED0')   
   Lugworms<-read.csv('https://git.io/vXESR')
   Estimates_females<-read.csv(file='https://git.io/vXgI8')
}
```
### 7.2 Prepare data
```
...
XY$Phi_females<-approx(x=Estimates_females$Female_weight_cov, y = Estimates_females$estimate, XY$Slope_Weight_gain_Females)$y

```
### 7.3. Prepare plot
Set device.
```{r}
if (Save_pdf) pdf(file='Figure 2_4 graphs_07_Nov_2016.pdf', useDingbats=FALSE, width=10, height=10)
...
Early_color<-'#6FC067'
```
### 7.4 Panel a - stayover change over years
```{r}
