...
write.csv(Snow50, file='Snow_remote.csv', row.names=FALSE)
```
...
lines(Y_m[,1]~XX, lwd=2, lty=1)
lines(Y_m[,2]~XX, lwd=2)
lines(Y_m[,3]~XX, lwd=2, lty=1)
...
Y_fem<- t(sapply(XX, FUN=function(x)
		 ...
		 probs=c(0.025, 0.5,0.975))))
lines(Y_fem[,1]~XX, lwd=2, lty=1)
lines(Y_fem[,2]~XX, lwd=2)
lines(Y_fem[,3]~XX, lwd=2, lty=1)
for (i in 1:length(Lugworms_vec)) {
