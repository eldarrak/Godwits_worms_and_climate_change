...
plot(jdate50~Yr, data=Snow_remote, pch=19, col=Colors[2], ylim=range(c(Snow_remote[,2], Snow_remote[,3])), las=1, xlab='Year', cex=2)
...
jags.params=c("Alpha_t", "sigma2_alpha", "sigma2_alpha_fem", "Int_lug", "Int_lug_fem", "Slope_lug", "Slope_lug_fem")
```
...
All_staging_data$log_Stopover<-scale(log((All_staging_data$departure-All_staging_data$arrival)), center=TRUE, scale=FALSE)
...
Alhpa_pred_f_df<-data.frame(estimate=Alpha_pred_f$fit, lci=Alpha_pred_f$fit-1.96*Alpha_pred_f$se.fit, uci=Alpha_pred_f$fit+1.96*Alpha_pred_f$se.fit)
Alhpa_pred_m_df<-data.frame(estimate=Alpha_pred_m$fit, lci=Alpha_pred_m$fit-1.96*Alpha_pred_f$se.fit, uci=Alpha_pred_m$fit+1.96*Alpha_pred_m$se.fit)
...
par(mar=c(4, 5,0,2 ))
hist(Stayovers$lugworms, breaks=seq(7.5, 22.5, by=2.5), main='', col=grey(0.9), las=1, ylab='', xlab='', xlim=c(7, 35), axes=FALSE )
...
names(Totals)<-c( 'jdate', 'total')
#par(xpd=TRUE)
hist(rep(Totals$jdate, Totals$total), xlim=xlims, axes=FALSE, xlab='', ylab='', main='', freq=TRUE, breaks=seq(100, 170, by=5))
axis(1, labels=Xlabseq, at=Label_jdates)
#axis(2, las=1, tcl=0.5, mgp=c(0, -3, 0),labels=seq(500, 2500, by=500), at=seq(500, 2500, by=500))
axis(2, las=1, tcl=0.5, mgp=c(0, -3, 0), )
...
abline(h=Arrivals_weights_cur$Female_arrival_weight-Arrivals_weights_cur$Female_arrival_weight_sigma, col=Fem_col, lwd=2, lty=2)
abline(h=Arrivals_weights_cur$Female_arrival_weight+Arrivals_weights_cur$Female_arrival_weight_sigma, col=Fem_col, lwd=2, lty=2)
...
par(xpd=TRUE)
plot(Mass ~ yday, data=aggr_data_females_Wad, pch=19, xlim=xlims, ylim=ylim_weights_f, axes=FALSE, xlab='', col=Fem_col)
axis(2, las=1, tcl=0.5, mgp=c(0, -3, 0))
axis(1, labels=Xlabseq, at=Label_jdates)
box(bty='l')
par(xpd=FALSE)
...
Fuleling_intercept_females<-Arrivals_weights_cur$Female_arrival_weight-Fueling_slopes_cur$Fuelling_slope_females*Arrivals_WZ_cur$arrival

abline(a=Fuleling_intercept_females, b=Fueling_slopes_cur$Fuelling_slope_females, col=Fem_col, lwd=2)
...
mtext('captures, n', side=1, line=2, cex=Cex)
   ...
   #   axis (4, las=1)
   rect(ybottom=c(hs$breaks[-length(hs$breaks)]), ytop=hs$breaks[-1], xleft=hs$counts, xright=0, border=Mal_col)
   #box(bty='l')

Arrivals_weights_cur<-subset(Arrival_weights, Yr==Year2plot)
abline(h=Arrivals_weights_cur$Male_arrival_weight, col=Mal_col, lwd=2)
abline(h=Arrivals_weights_cur$Male_arrival_weight+Arrivals_weights_cur$Male_arrival_weight_sigma, col=Mal_col, lwd=2, lty=2)
abline(h=Arrivals_weights_cur$Male_arrival_weight+Arrivals_weights_cur$Male_arrival_weight_sigma, col=Mal_col, lwd=2, lty=2)
#------------------------------------------
...
plot(Mass ~ yday, data=aggr_data_males_Wad, pch=19, xlim=xlims, ylim=ylim_weights_f, axes=FALSE, xlab='', col=Mal_col)
axis(2, las=1, tcl=0.5, mgp=c(0, -3, 0))
axis(1, labels=Xlabseq, at=Label_jdates)
...
abline(a=Fuleling_intercept_males, b=Fueling_slopes_cur$Fuelling_slope_males, col=Mal_col, lwd=2)
