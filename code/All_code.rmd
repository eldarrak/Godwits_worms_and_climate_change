...
summary(lm(jdate50~Yr, data=subset(Res, Yr>1995)))
```
...
# tipula#8783ce
Tipula_color<-'#cbce83'
points(yday~Yr, data=Tipula, pch=21, bg=Tipula_color, lwd=1, cex=2, col=NULL )
lm_tipula<-lm(yday~Yr+Region, data=Tipula)
lines(predict(lm_tipula, newdata=data.frame(Yr=c(1992,2016), Region='South'))~
              c(1992,2016),lwd=2, col=Tipula_color)
lines(predict(lm_tipula, newdata=data.frame(Yr=c((2004-0.5):(2007+0.5)), Region='North'))~
              c((2004-0.5):(2007+0.5)), lwd=2, col=Tipula_color, lty=2)
# nests
points(yday~Yr, data=Nests, pch=21, cex=2,  lwd=1, bg='#cc79a7', col=NULL)
...
points(yday~Yr, data=Arrivals_Khat,  pch=21, cex=2, bg=Khat_arr_color, lwd=1, col=NULL)
# Wadden arrivals..
Width=0.25
...
hist(Stayovers$lugworms_ad, main='', xlab='', ylab='n years', las=1, col='#6FC067',
     axes=FALSE, xaxs='i', xlim=range(XY[1]))
