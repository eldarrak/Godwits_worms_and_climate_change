...
if (Use_local_data) {
   ...
   download.file('https://git.io/vXgvd', destfile='./results/female_survival_prediction.csv')
}
...
Page<-getURL('https://www.ncei.noaa.gov/data/snow-cover-extent/access/',ssl.verifypeer = FALSE, dirlistonly = TRUE, ftp.use.epsv = FALSE)
filename<-paste0('https://www.ncei.noaa.gov/data/snow-cover-extent/access/nhsce_v01r01_19661004_' , substr(strsplit(Page, 'nhsce_v01r01_19661004_')[[1]][2], start=1, stop=11))
...
### 1.4 Select and plot snow points
```{r}
...
### 1.6 Comparison of remote sensing data with ground observations
```{r}
...
cat('Bayesian p-value = ', mean(migration_model$BUGSoutput$sims.list$fit.new<migration_model$BUGSoutput$sims.list$fit), '\n')
cat('lack of fit ratio = ', mean(migration_model$BUGSoutput$sims.list$fit)/mean(migration_model$BUGSoutput$sims.list$fit.new), '\n')
...
## 3. Casuality in phenological data with structural equations modelling
...
SEM_coefs<-cbind('b'=sapply(phenology_SEM$BUGSoutput$sims.list[Parameter.Index], FUN=function(x) mean(x)), 'P(|b|>0)'=sapply(phenology_SEM$BUGSoutput$sims.list[Parameter.Index], FUN=function(x) max(mean(x>0), mean(x<0))))
print(SEM_coefs)
...
for (i in 1:length(Individuals)) {
   ...
   Index_stationary[which(spDists(cbind(Track$lon, Track$lat), longlat=TRUE, segments=TRUE)>25)]<-0 # more then 25 km => movement
   Starts<-unique(c(1, which(diff(Index_stationary)==1)+1))
   Ends<-unique(c(which(diff(Index_stationary)==-1)+1, length(Index_stationary)))
   Periods<-data.frame(Start_Index=Starts, End_Index=Ends, Start_time=Track$Time[Starts], End_time=Track$Time[Ends])
   ...
   Periods$Mean_lon<-sapply(1:nrow(Periods), FUN=function(x) mean(Track$lon[Periods$Start_Index[x]: Periods$End_Index[x]]))
   Periods$Mean_lat<-sapply(1:nrow(Periods), FUN=function(x) mean(Track$lat[Periods$Start_Index[x]: Periods$End_Index[x]]))
   Periods$Year= Year

   # and now I want to exclude ones in the wintering, Wadden Sea or breeding grounds
   Periods<-Periods[!(Periods$Mean_lat<21.5 | (Periods$Mean_lat>52.4 & Periods$Mean_lat<56 & Periods$Mean_lon > 4 & Periods$Mean_lon < 10) | Periods$Mean_lon > 80),]
   if (nrow(Periods)>=1) {
...
Fuelling_slopes<-data.frame(Yr=Years,
         ...
         Fuelling_slope_males_predicted=11*Lugworms_vec/(fuelling_model$BUGSoutput$mean$K_sat_m[1]+Lugworms_vec),
         Fuelling_slope_females_predicted=13.2*Lugworms_vec/(fuelling_model$BUGSoutput$mean$K_sat_f[1]+Lugworms_vec))
...
All_staging_data$log_fuelling_slope_females_st<-scale(log(All_staging_data$Fuelling_slope_females), center=TRUE, scale=FALSE)
All_staging_data$log_fuelling_slope_males_st<-scale(log(All_staging_data$Fuelling_slope_males), center=TRUE, scale=FALSE)
...
Phi.5=list(formula=~two_plus + log_fuelling_slope*SEX)
...
### 7.5 Plot model results for panel C in Fig.1
```{r fig.width=5, fig.height=5, results='hide'}
print(Counts_model)
if (Save_pdf) pdf("Mauritania_counts.pdf", 5, 5, useDingbats=FALSE)
...
points(rowSums(y.matrix)~ Years, pch=21, bg='#0072B2', cex=2, col=NULL)
if (Save_pdf) dev.off()
```

## 8. Phenology plot (Fig. 1 b)
...
### 8.2 Estimate linear slopes of phenological events in time
```{r}
summary(lm(yday~Yr, data=subset(Snow, Yr %in% c(1994:2016))))
summary(lm(yday~Yr, data=subset(Tipula, Region =='South')))
summary(lm(yday~Yr, data=subset(Nests, Region =='South')))
...
summary(lm(Stayover~Yr, data=Stayovers))
# for all twenty years it is going to be
summary(lm(Stayover~I(Yr/20), data=Stayovers))
```
...
par(xpd=FALSE)
# snow
abline(lm(yday~Yr, data=subset(Snow, Yr %in% c(1994:2016))), col='#7bccc4', lwd=2)
# tipula#8783ce
Tipula_color<-'#8783CE'
...
# nests
points(yday~Yr, data=subset(Nests, Region=='South'), pch=21, cex=2,  lwd=1, bg='#cc79a7', col=NULL)
lm_nests<-lm(yday~Yr, data=subset(Nests, Region=='South'))
lines(predict(lm_nests, newdata=data.frame(Yr=c(1992,2016)))~
              c(1992,2017), lwd=2, col='#cc79a7')
# Khat arrivals
Khat_arr_color<-'#cbce83'
...
rect(xleft=Stayovers$Yr-Width, xright=Stayovers$Yr+Width, 
     ...
     border=NA, col='#fdb194')
abline(lm(yday~Yr, data=Arr_Wadden), col='#FC8D62', lwd=2)
points(yday~Yr, data=Arr_Wadden, bg='#FC8D62', pch=21, cex=2, col=grey(0.9))
box()
if (Save_pdf) dev.off()
```
...
```{r}
Scale_females<-scale(log(All_staging_data$Fuelling_slope_females))
Scale_males<-scale(log(All_staging_data$Fuelling_slope_males))

Prediction.matrix<-expand.grid(log_fuelling_slope=seq(1.25-1.717, 2.35-1.717, by=0.001), log_Stopover=seq(2.75, 3.6, by=0.001))
...
Prediction.matrix$Prediction_females<-plogis(Design_females%*%as.matrix(summary(Model)$beta[,1, drop=FALSE]))
Prediction.matrix$Prediction_males<-plogis(Design_males%*%as.matrix(summary(Model)$beta[,1, drop=FALSE]))
Prediction.matrix$fuelling_slope_females<-exp(Prediction.matrix$log_fuelling_slope+attr(Scale_females,"scaled:center"))
Prediction.matrix$Stopover<-exp(Prediction.matrix$log_Stopover)			
Prediction.matrix$fuelling_slope_males<-exp(Prediction.matrix$log_fuelling_slope+attr(Scale_males,"scaled:center"))
...
### 9.3 Female plot
First I  plot female figure (Figure 2) and then male figure for extended data Fig 2.
...
Prediction.matrix_cur<-subset(Prediction.matrix,  Stopover>=17 &  Stopover<=35 & fuelling_slope_females<=10.5 & fuelling_slope_females>=4)
...
par(xpd=TRUE)
mtext("refuelling rate (g/d)", side=1, line=3, cex=axes.labels.cex)
par(xpd=FALSE)
...
contour(Female_Image,add=T, levels=plogis(Design_females_s%*%as.matrix(summary(Model)$beta[,1, drop=FALSE]))[1], lwd=3, col=grey(0.5), labels=round(plogis(Design_females_s%*%as.matrix(summary(Model)$beta[,1, drop=FALSE]))[1],2), labcex = 1)

contour(Female_Image,add=T, levels=c(0.8, 0.85, 0.9, 0.95), col=grey(0.5), labcex=1)

points(Stayover~Fuelling_slope_females, data=Stayovers, pch=21, col='white', bg='black', cex=2)
box()
...
par(mar=c(4, 5,2,2 ))
plot(Stayovers$Fuelling_slope_females~Stayovers$Yr, type='p',  ylab='',
     las=1, xlim=c(1992, 2018), xlab='', axes=F, yaxs='i',  cex=2, pch=21, bg='black', col='white', las=1, ylim=c(4,10.5))
...
abline(lm(Fuelling_slope_females~Yr, data=Stayovers), lwd=2)
...
points(Stayovers$Fuelling_slope_females~Stayovers$Yr, cex=2, pch=21, bg='black', col='white')
...
par(mar=c(4, 5,2,2 ))
par(xaxs='i')
plot(Stayovers$Stayover~Stayovers$Fuelling_slope_females, type='p',  ylim=c(17,35), xlim=c(4,10.5) ,ylab='',
...
par(xpd=TRUE)
mtext(side=1, 'refuelling rate (g/d)', line=3, cex=axes.labels.cex)
par(xpd=FALSE)

axis(2,cex.axis=axes.labels.cex, las=1)
mtext(side=2, 'refuelling time (d)', line=3,cex=axes.labels.cex)

abline(lm(Stayover~Fuelling_slope_females, data=Stayovers), lwd=2)
# 95%CI

XX<-seq(4,10.5, by=0.1)

Pred<-predict(M5, newdata=data.frame(Fuelling_slope_females=XX), se.fit=TRUE)
UCI<-Pred$fit+1.96*Pred$se.fit
LCI<-Pred$fit-1.96*Pred$se.fit
lines(UCI~XX, lty=2)
lines(LCI~XX, lty=2)
...
plot(lugworms_ad~Yr, data=Lugworms, type='p',  ylab='',
     las=1, xlim=c(1992, 2018), xlab='', axes=F, yaxs='i',  cex=2, pch=21, bg='black', col='white', ylim=c(min(Lugworms$lugworms_ad, na.rm=TRUE)-0.5, max(Lugworms$lugworms_ad, na.rm=TRUE)+0.5))
...
# Now panel 6
par(mar=c(4, 5,2,2 ))
plot(fuelling_model$BUGSoutput$mean$Alpha_t[,2]~Lugworms$lugworms_ad,
     ylim=c(4,8.5),ylab='',
...
points(fuelling_model$BUGSoutput$mean$Alpha_t[,2]~Lugworms$lugworms_ad,
...
if (Save_pdf) dev.off()
```
### 9.5 Check for temporal autocorrelation
...
M2<-lm(Fuelling_slope_females~Yr, data=Stayovers)
Stayovers$resid_fsf<-NA
Stayovers$resid_fsf[!is.na(Stayovers$Fuelling_slope_females)]<-residuals(M2)

acf(Stayovers$resid_fsf,  na.action = na.pass)

M2_gls<-gls(Fuelling_slope_females~Yr, data=Stayovers, na.action = na.omit)
M2_gls_AR1<-gls(Fuelling_slope_females~Yr, data=Stayovers, na.action = na.omit, correlation = corAR1(form = ~ Yr))
...
########################################
# fuelling by lugworms
 
M4<-lm(Fuelling_slope_females~lugworms_ad, data=Stayovers)

Stayovers$resid_M4<-NA
Stayovers$resid_M4[!is.na(Stayovers$lugworms_ad) & !is.na(Stayovers$Fuelling_slope_females)]<-residuals(M4)

acf(Stayovers$resid_M4,  na.action = na.pass)

M4_gls<-gls(Fuelling_slope_females~lugworms_ad, data=Stayovers, na.action = na.omit)
M4_gls_AR1<-gls(Fuelling_slope_females~lugworms_ad, data=Stayovers, na.action = na.omit, correlation = corAR1(form = ~ Yr))
...
Prediction.matrix_cur_males<-subset(Prediction.matrix,  fuelling_slope_males>=3.5 &  Stopover<=35 & fuelling_slope_males<=8.5 & Stopover>17)

Male_Image<-as.image(x=Prediction.matrix_cur_males[,c(7,6)], Z=Prediction.matrix_cur_males$Prediction_males, nx=200, ny=200)
...
contour(Male_Image, add=T, levels=plogis(Design_males_s%*%as.matrix(summary(Model)$beta[,1, drop=FALSE]))[1], lwd=3, col=grey(0.5), labels=round(plogis(Design_males_s%*%as.matrix(summary(Model)$beta[,1, drop=FALSE]))[1],2), labcex = 1)

contour(Male_Image,add=T, levels=c(0.8, 0.85, 0.9, 0.95), col=grey(0.5), labcex=1)
	  
points(Stayover~Fuelling_slope_males, data=Stayovers, pch=21, col='white', bg='black', cex=2)
box()
...
par(mar=c(4, 5,2,2 ))
plot(Stayovers$Fuelling_slope_males~Stayovers$Yr, type='p',  ylab='',
     las=1, xlim=c(1992, 2018), xlab='', axes=F, yaxs='i',  cex=2, pch=21, bg='black', col='white', las=1, ylim=c(3.5,8.5))
...
axis(2, las=1, cex.axis=axes.labels.cex)
mtext(side=2, 'refuelling rate (g/d)', line=3,cex=axes.labels.cex)
abline(lm(Fuelling_slope_males~Yr, data=Stayovers), lwd=2)
...
points(Stayovers$Fuelling_slope_males~Stayovers$Yr, cex=2, pch=21, bg='black', col='white')
...
par(mar=c(4, 5,2,2 ))
par(xaxs='i')
plot(Stayovers$Stayover~Stayovers$Fuelling_slope_males, type='p',  ylim=c(17,35), xlim=c(3.5,8.5) ,ylab='',
     las=1, xlab='', cex=2, axes=F, yaxs='i', pch=21, col='white', bg='black')
...
abline(lm(Stayover~Fuelling_slope_males, data=Stayovers), lwd=2)
# 95%CI

XX<-seq(3.5,8.5, by=0.1)

Pred<-predict(M6, newdata=data.frame(Fuelling_slope_males=XX), se.fit=TRUE)
UCI<-Pred$fit+1.96*Pred$se.fit
LCI<-Pred$fit-1.96*Pred$se.fit
lines(UCI~XX, lty=2)
lines(LCI~XX, lty=2)
...
# Now panel 6
par(mar=c(4, 5,2,2 ))
plot(fuelling_model$BUGSoutput$mean$Alpha_t[,1]~Lugworms$lugworms_ad,
     ylim=c(3.5,8.5),ylab='',
     xlab='' ,las=1, type='n', axes=FALSE)
...
points(fuelling_model$BUGSoutput$mean$Alpha_t[,1]~Lugworms$lugworms_ad,
   pch=21, cex=2, bg='black', col='white')
...
## 10. Plot capture data with slopes (Extended data Figure 2 and 3)
...
## 10.2 Prepare data for plotting
```{r}
   ...
   Arrivals<-merge(Arrivals, Fuelling_slopes[,1:3])
   Arrivals$Male_fuelling_intercept<-  Arrivals$Male_arrival_weight-
                                        Arrivals$Fuelling_slope_males*Arrivals$arrival
   Arrivals$Female_fuelling_intercept<-Arrivals$Female_arrival_weight-
                                        Arrivals$Fuelling_slope_females*Arrivals$arrival
   ...
   Arrivals$Male_Departure_Weight<-Arrivals$Male_fuelling_intercept+
                                       Arrivals$Fuelling_slope_males*Arrivals$Departure
   Arrivals$Female_Departure_Weight<-Arrivals$Female_fuelling_intercept+
                                       Arrivals$Fuelling_slope_females*Arrivals$Departure
...
```{r fig.width=10, fig.height=10}
   ...
   geom_abline(data=Arrivals, aes(intercept=Male_fuelling_intercept,
                 slope=Fuelling_slope_males), col='#D55E00', lwd=Lwd) +
...
```{r fig.width=10, fig.height=10}
   ...
   geom_abline(data=Arrivals, aes(intercept=Female_fuelling_intercept,
                                  slope=Fuelling_slope_females), col='#D55E00', lwd=Lwd) +
   geom_vline(data=subset(Arrivals), aes(xintercept = Departure), col="#56B4E9", lwd=Lwd) +
...
```{r fig.width=2, fig.height=2}
   ...
   annotate("segment", x=X_labels-0.1, xend=X_labels+0.1, y=-0.25-0.075, yend=-0.25+0.075,
            col='#D55E00', lwd=1.5)+
   annotate("text", x=X_text, y = -0.25, label = "refuelling slope", hjust = 0) +
