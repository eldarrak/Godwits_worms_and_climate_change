## Validation of use of Balgzand data on lugowrm abundance for inference on _taymyrensis_ Bar-Tailed godwit fuelling rates

_this is internal report authorised by Eldar Rakhimberdiev_

Problem description:
In the mansucript I wrote and would like to submit I used data on lugworm abundance from Balgzand to correlate it with fuelling rates of taymyrensis godwits. The question rased at the department is whether it is correct as we know that our godwits are not abundand there as they prefer Terschelling coast [Duijns et al. 2009](#duijns_2009).
Solution:
SIBES dataset is the only one that covers Terschelling coast, it also covers Balgazand making it possible to directly compare whether the two areas have correlated in time changes in Lugworm abundance. Unfortunately SIBES covers only 2008:2014. 2015 and 2016 sampling years are not yet processed.
...
```{r}
Balgzand_samples_all<-data.frame(Place='B', sample_id=Balzand_utm@data$sample_id, sampleyear=Balzand_utm@data$sampleyear)
Terschelling_samples_all<-data.frame(Place='T', sample_id=SpPoints_utm@data$sample_id[Selected_Points[1,]], sampleyear=SpPoints_utm@data$sampleyear[Selected_Points[1,]])
...
Lugworms_count_ad_aggr<-aggregate(Lugworms_count_ad$n_cruc_ceiling, by=list(sample_id=Lugworms_count_ad$sample_id, Place=Lugworms_count_ad$Place, sampleyear=Lugworms_count_ad$sampleyear), FUN=sum)
```
# Estimate mean by area/year
```{r}
tmp<-aggregate(Lugworms_count_ad_aggr$x, by=list(sampleyear=Lugworms_count_ad_aggr$sampleyear, Place=Lugworms_count_ad_aggr$Place), mean)
Res<-data.frame(sampleyear=2008:2014, Terschelling=subset(tmp, Place=='T')$x/0.01731803, Balgzand=subset(tmp, Place=='B')$x/0.01731803) # convert to square meters
```

### Plot results
```{r}
plot(Terschelling~Balgzand, ylab='density Terschelling', xlab='density Balgzand', type='n', data=Res)
text(x=Res$Balgzand, y=Res$Terschelling,  ylab='density Terschelling', xlab='density Balgzand',  labels=seq(2008,2014))
abline(lm(Terschelling~Balgzand, data=Res))
cor.test(subset(tmp, Place=='T')$x,subset(tmp, Place=='B')$x)
```
### Conclusion

From here I conclude that data on Terchelling and Balgzand are highly correlated (Pearson correlation coefficient = 0.86). This means that despite difference between two areas in absolute abundance of lugworms they vary between years in the same way. 

### References

1. Duijns, S. et al. Foraging dite delection of two subspecies of bar-tailed godwit _Limosa lapponica_: time minimizers accept greater predation danger than energy minimizers. Ardea 97, 51–59 (2009). <a id="duijns_2009"></a>